...
show_words.onreadystatechange=function(){
    if(show_words.readyState==XMLHttpRequest.DONE && show_words.status==200){
        ...
        for(x=0;x<=rows;x++){
            ...
            '<span id="comment'+true_id+'"'+'>'+all_info[x].comments+'</span>'+'</td>'+'<td style="text-align:center">'+true_name+'</td>'+'<td>'+'&nbsp'+'&nbsp'+all_info[x].time+'</td>'+'<td style="text-align:center">'+'&nbsp'+'<button onclick=change_words('+true_id+','+x+')'+'>'+"修改"+'</button>'+'</td>'+
            '<td>'+'<button onclick=delete_words('+true_id+')'+'>'+"删除"+'</button>'+'</td>'+'</table>'+'</p>'+'<br>';
